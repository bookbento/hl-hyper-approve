...
async function recomputeExtraLineStatus(lineId: number): Promise<ExtraStatus> {
  ...
  const formattedStatus = final.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ');
...
async function hasActiveExtraLine(memoId: number): Promise<boolean> {
...
export const appendExtraApprovers: RequestHandler = async (req, res) => {
  ...
  // ===== อนุญาต “ผู้มีส่วนร่วมทุกคน” (Owner + LoA ทุกเลเวลของเวอร์ชันล่าสุด + CC + ผู้ที่อยู่ใน Extra line + ผู้สร้าง Extra line) =====
  ...
  if (line.status === ExtraStatus.REJECTED) {
    res.status(409).json({ error: "ไลน์นี้ถูก Reject แล้ว ไม่สามารถเพิ่มได้" });
    return;
  }
  ...
  // กันซ้ำ/กันรายชื่อที่ไม่อนุญาต
  const existed = new Set<number>(line.approvers.map((a) => a.userId));

  // ใช้ชุดจากด้านบนต่อได้เลย (ไม่ต้องยิงซ้ำ)
  ...
  const toAdd = candidates.filter(
    (uid) => !approverUserIds.has(uid), // กันเฉพาะคนที่เป็น LoA หลักแล้ว
  );
  ...
  // ⛳ Email + Notification + History (ระบุชื่อ)
  try {
    ...
    // แจ้งเตือนไปที่ Owner และคนใหม่
    const ownerId = memo.userId;
    const receiverIds = ownerId ? Array.from(new Set([ownerId, ...toAdd])) : toAdd;

    const { attempted, okCount, failCount } =
      await sendExtraApprovalEmailsAsync({
        receiverIds, // ส่งให้ Owner และคนใหม่
        memoSubject: memo.subject || `#${memoId}`,
        memoNumber: memo.memonumber ?? memoId,
        memoId,
        actorName,
        mode: "added", 
        addedNames: addedNames, // ส่งรายชื่อคนถูกเพิ่มไปในอีเมล
        ownerId
      });

    // กรองคนที่จะส่ง in-app notification ตาม preference
    ...
    if (addedUsers.length) {
      const notifyAdded = addedUsers.filter((u) => u.id !== ownerId && notifySet.has(u.id));
...
export const createExtraApprovalLine: RequestHandler = async (req, res) => {
  ...
  // ✅ NEW: เช็คว่าถูก Mention ใน comments หรือไม่
  const isMentioned = !!(await prisma.commentTag.findFirst({
    where: { memoId, userId: actorId },
    select: { id: true },
  }));

  // ✅ NEW: เช็คว่าเป็น Extra Approver (เคยอยู่ใน extra line ของ memo นี้) หรือไม่
  ...
  preApprovedUsers.forEach((u) => {
    // แปลง actedAt เป็น Date ถ้ามีค่า
  ...
  const approversData = userIds.map((uid) => {
    ...
    // ถ้าเป็นคนที่เคยอนุมัติแล้ว ให้ใช้เวลาเดิม (สำคัญ!) - ถ้าไม่มีเวลาเดิมก็ใช้ now
    const originalActedAt = preApprovedMap.get(uid);

    return {
      userId: uid,
      // ถ้าเป็นคนที่ approved แล้ว ให้ set status เป็น Approved ทันที
  ...
  const line = await prisma.extraApprovalLine.create({
    ...
    include: {
      ...
      comment: true, // ✅ รวม comment ในผลลัพธ์ (ถึงปจบจะทำเป็น array แล้ว)
    },
  });
  ...
  const commentsData = userIds.map((uid) => ({
    memoId,
    userId: actorId, // คนที่พิมพ์
    comment: commentText || "",
    ExtraUserid: uid, // คนที่被อ้างถึง (Extra Approver)
    ExtraStatus: preApprovedMap.has(uid) ? "Approved" : "Pending", // สถานะเริ่มต้น
  ...
  try {
    ...
    // ดึงชื่อผู้กระทำให้ได้ "(nickname)" ถ้ามี
    ...
    const ownerId = memo.userId;
    const receiverIds = ownerId ? Array.from(new Set([ownerId, ...userIds])) : userIds;

    // ส่งเมลให้ Owner และคนใหม่
    const { attempted, okCount, failCount } =
      await sendExtraApprovalEmailsAsync({
        receiverIds, 
        memoSubject: memo.subject || `#${memoId}`,
    ...
    const allIds = [ ownerId ? ownerId : -1, ...recipients.map(u => u.id) ].filter(id => id > 0);
    const filteredNotificationIds = await filterUsersForEmail(
      allIds,
      notificationSlug
    );
    ...
    // 🔔 in-app notifications ไปยัง Owner
    ...
    // 🔔 in-app notifications ไปยังคนใหม่
    if (recipients.length > 0) {
      const notifyRecipients = recipients.filter((u) => u.id !== ownerId && notifySet.has(u.id));
    ...
    // 📝 history — บันทึกชื่อคนที่ถูกเพิ่ม (เฉพาะคนใหม่!)
    const preApprovedSet = new Set(
      preApprovedUsers.map((u) => Number(u.userId)),
    );
    const newRecipients = recipients.filter((u) => !preApprovedSet.has(u.id));

    // ถ้าไม่มีคนใหม่เลย (case rare)
    if (newRecipients.length === 0) {
      // อาจจะไม่ต้อง log หรือ log แบบ info
      // แต่โดย flow ปกติต้องมีคนใหม่เสมอ
    }
    ...
    // fallback history แบบสั้น ๆ ตาม format ใหม่
    await prisma.memoHistory.create({
      data: {
        memoId,
        userId: actorId,
...
export const actOnExtraApprovalLine: RequestHandler = async (req, res) => {
  ...
  const target = await prisma.extraApprover.findFirst({
    where: { extraId: lineId, userId: actorId }, // ใช้ field ตามสคีมา: extraId
    include: { extra: true }, // ความสัมพันธ์ชื่อ extra
  });
  if (!target) {
  ...
  // ✅ line ต้องยัง active (PENDING/IN_PROGRESS) — ใช้ Set<ExtraStatus> กัน TS union error
  ...
  // ✅ อัปเดตสถานะของคนกด (ใช้ตาราง Status)
  ...
  if (statusCode === "rejected") {
    ...
    // ✅ ดึง owner ของ memo เพื่ออัปเดตสถานะรวมที่แถวของ owner (เหมือน evaluateAndUpdateMemoStatus)
    const memo = await prisma.masterMemo.findUnique({
      where: { id: memoId },
      select: { userId: true },
    });
    ...
    // ✅ อัปเดตสถานะรวมของ memo ที่แถว owner → Rejected (เหมือน upsertStatusWithHistory)
    // กัน unique constraint (memoId, userId, statusId): ลบแถวที่ซ้ำก่อน
...
export const getActiveExtraApprovalLine: RequestHandler = async (req, res) => {
  ...
  // ⬇️ ถ้าไม่เจอ active ให้คืนล่าสุด (COMPLETED/REJECTED) พร้อม approvers
  if (!line) {
  ...
  res.json({
    ...
    comments: line.comment, // ✅ ส่งกลับเป็น Array
    approvers,
  });
};
...
// Body: { memoIds: number[] }
// Response: Record<memoId, { id, status, createdAt, approvers[] } | null>
//
// Bulk replacement for the per-memo GET version above. For each memo, the response
// reproduces the same selection rule: prefer the latest active line (PENDING or
// IN_PROGRESS); otherwise return the latest line of any status; if no line exists,
// return null. Visibility is enforced with the same rules as getAllMemos.
export const getActiveExtraApprovalLinesBulk: RequestHandler = async (
  ...
  try {
    ...
    // 1) Fetch all extra approval lines for the visible memos in one query.
    //    Order by id DESC so the first line we encounter per (memoId, "active"-or-not)
    //    bucket is the latest. We then pick: latest active else latest of any status.
    ...
    // Pick preferred line per memo: active first, then latest of any status.
    ...
    // 2) Bulk fetch approvers for the chosen lines.
    ...
    // 3) Build response. Memos with no line → null. Memos not allowed → omitted.
    ...
    for (const id of allowedIds) {
      ...
      // Strip extraId before returning to match the shape of the single endpoint.
...
export const removeExtraApprovalLine: RequestHandler = async (req, res) => {
  ...
  if (line.createdById !== actorId) {
    res.status(403).json({ error: "Only the creator can remove this approval line" });
    return;
  }

  if (line.status !== "PENDING") {
    res.status(409).json({ error: "Only PENDING approval lines can be removed" });
    return;
  }
  ...
  // Collect recipients for notifications
  const extraApproverIds = line.approvers.map(a => a.userId).filter((id): id is number => id !== null);
  ...
  for (const uid of extraApproverSet) {
    if (notifyExtraSet.has(uid)) {
      notis.push(buildNoti(uid, `${actorLabel} removed your extra approval line from "${subj}".`));
    }
  }
  for (const uid of ownerSet) {
    if (notifyOwnerSet.has(uid)) {
      notis.push(buildNoti(uid, `${actorLabel} removed an extra approval line (${extraNames.join(", ")}) from "${subj}".`));
    }
  }
  ...
  // Delete associated comment first (if any)
  if (line.commentId) {
    await prisma.comment.delete({ where: { id: line.commentId } }).catch(() => {});
  }

  // Delete the extra approval line (ExtraApprovers cascade)
        ...
        for (const uid of others) {
          if (notifyOthers.has(uid)) {
            if (mentionedNames.length > 0) {
              notis.push(
                buildNoti(uid, `${actorDisp} mentioned ${mentionedNames.join(", ")} in a comment on "${subj}".`),
              );
            } else {
              notis.push(
...
/* ----------------------------------------------------------- */
/*  Send extra-approval e-mails (with login token)             */
/* ----------------------------------------------------------- */
export async function sendExtraApprovalEmailsAsync({
  receiverIds,
  memoSubject,
  memoNumber,
  memoId,
  ...
  addedNames, // รายชื่อคนที่ถูกขอ/เพิ่ม
  ownerId, // ระบุว่าใครคือเจ้าของ
}: {
  receiverIds: number[];
  memoSubject: string;
  memoNumber: string | number;
  ...
  try {
    // ✅ guard: ข้ามส่งเมลถ้า memo อยู่สถานะเงียบ (Draft/Recall) — ให้พฤติกรรมเหมือน comment
    const latestPivot = await prisma.memoStatusPivot.findFirst({
      where: { memoId },
      orderBy: { id: "desc" },
      include: { status: { select: { id: true, name: true } } },
    ...
    if (!filteredReceiverIds.length) return { attempted: 0, okCount: 0, failCount: 0 };
    ...
    const results = await Promise.allSettled(
      sendTo.map(async (u) => {
        const token = await makeEmailToken(u.id, memoId); // pattern เดียวกับ comment
        const viewLink = `${FRONTEND_URL}/memo/${memoId}`;
...
function buildExtraHtml(
  ...
  const addedUsersStr = addedNames?.length ? ` (<b>${addedNames.join(", ")}</b>)` : "";
...
export const deleteComment: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    if (commentData.tags.length === 0) {
      res.status(403).json({ error: "Only comments with mentions can be deleted." });
      return;
    }
    ...
    const fmtName = (u: { name?: string | null; lastname?: string | null; nickname?: string | null }) => {
      const base = [u.name, u.lastname].filter(Boolean).join(" ").trim();
      return u.nickname ? `${base} (${u.nickname})` : base || (u.name ?? "");
    };
    ...
    for (const uid of mentionRecipients) {
      if (notifyMentions.has(uid)) {
        notis.push(
          buildNoti(uid, `${actorDisp} removed a mention of you in memo "${subj}".`),
        );
      }
    }
    for (const uid of others) {
       if (notifyOthers.has(uid)) {
         notis.push(
           buildNoti(uid, `${actorDisp} removed a comment mentioning ${mentionedNames.join(", ")} in "${subj}".`),
         );
       }
    }
    if (notis.length) await prisma.notification.createMany({ data: notis });
    ...
    // Send Emails
    const snippetText = commentData.comment ? commentData.comment.slice(0, 120) : "[attachment]";
...
/* ----------------------------------------------------------- */
/*  Send deleted-extra-approval e-mails                        */
/* ----------------------------------------------------------- */
export async function sendDeleteExtraApprovalEmailsAsync({
  receiverIds,
  memoSubject,
  memoNumber,
  memoId,
    ...
    await Promise.allSettled(
      sendTo.map(async (u) => {
        ...
        return sendEmail(
          [u.email!],
          subjectLine,
          buildDeleteExtraPlain(u.name, actorName, memoSubject, memoNumber, viewLink, deletedNames, isOwner),
          buildDeleteExtraHtml(u.name, actorName, memoSubject, memoNumber, viewLink, deletedNames, isOwner),
          [],
        );
      }),
    );
...
function buildDeleteExtraPlain(
  ...
  const deletedUsersStr = deletedNames.length ? ` (${deletedNames.join(", ")})` : "";
  const lead = isOwner
...
function buildDeleteExtraHtml(
  ...
  const deletedUsersStr = deletedNames.length ? ` (<b>${deletedNames.join(", ")}</b>)` : "";
  const leadHtml = isOwner
    ...
    await Promise.allSettled(
      sendTo.map(async (u) => {
        ...
        return sendEmail(
          [u.email!],
          subjectLine,
          buildDeleteCommentPlain(u.name, commenter, snippet, memoNumber, viewLink, isMention, mentionedNames),
          buildDeleteCommentHtml(u.name, commenter, snippet, memoSubject, memoNumber, viewLink, isMention, mentionedNames),
          [],
        );
      }),
    );
